...
import { Plus, Search, Trash2, ViewIcon } from "lucide-react";
...
export function FineManagement() {
  ...
  const handleDeleteFine = async (id?: string | null) => {
    const fineId = id ?? deleteId;
    if (!fineId) return;
    ...
    <div className="space-y-6">
                      ...
                      <td className="py-3 px-4">
                        <div className="flex gap-2">
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            className="border-red-300 text-red-600 hover:bg-red-50 bg-transparent hover:text-red-600 cursor-pointer"
                            onClick={() => setDeleteId(fine?._id)}
                          >
      ...
      <ConfirmModal
        ...
        onConfirm={async () => {
          const id = deleteId;
          if (!id) {
            toast.error("No fine selected");
            return;
          }
          await handleDeleteFine(id);
        }}
      />
    </div>
  );
